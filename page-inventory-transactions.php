...
<form class="inventory-filters inventory-filters--transactions" method="get" data-inventory-dependent>
...
<label><span>پروژه</span><select name="project_id" data-inventory-project><option value="">همه پروژه‌ها</option><?php foreach ($projects as $project): ?><option value="<?php echo (int)$project->ID; ?>" <?php selected($project_id,$project->ID); ?>><?php echo esc_html(get_the_title($project)); ?></option><?php endforeach; ?></select></label>
