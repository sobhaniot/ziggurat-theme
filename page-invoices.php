...
$copy_from_id = isset($_REQUEST['invoice_copy_from'])
    ? absint($_REQUEST['invoice_copy_from'])
    : absint($_REQUEST['copy_from'] ?? ($_REQUEST['copied_from_invoice_id'] ?? 0));
$form_error = null;
$settings_error = null;
$list_error = null;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['restore_invoice'])) {
    ...
    $trash_record = zigurat_invoice_trash_get_record($trash_id);
    if ($trash_record) {
        $brand = $trash_record->brand;
    }
    $nonce = isset($_POST['invoice_restore_nonce']) ? sanitize_text_field(wp_unslash($_POST['invoice_restore_nonce'])) : '';
    if (!wp_verify_nonce($nonce, 'zigurat_restore_invoice_' . $trash_id)) {
        $list_error = new WP_Error('invalid_nonce', 'درخواست بازیابی معتبر نیست؛ صفحه را تازه‌سازی کنید.');
    } else {
        $restore_with_new_number = !empty($_POST['restore_with_new_number']);
        $restored_invoice = zigurat_invoice_restore_from_trash($trash_id, $restore_with_new_number);
        if (is_wp_error($restored_invoice)) {
            $list_error = $restored_invoice;
        } else {
            $restore_status = !empty($restored_invoice->_restored_with_new_number) ? 'restored-renumbered' : 'restored';
            wp_safe_redirect(add_query_arg(array(
                'invoice-status' => $restore_status,
                'restored-number' => zigurat_invoice_object_number($restored_invoice),
            ), zigurat_invoice_page_url(array('brand'=>$restored_invoice->brand, 'view'=>'trash'))));
            exit;
        }
    }
    $view = 'trash';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_invoice_permanently'])) {
    $trash_id = absint($_POST['trash_id'] ?? 0);
    $trash_record = zigurat_invoice_trash_get_record($trash_id);
    if ($trash_record) {
        $brand = $trash_record->brand;
    }
    $nonce = isset($_POST['invoice_permanent_delete_nonce']) ? sanitize_text_field(wp_unslash($_POST['invoice_permanent_delete_nonce'])) : '';
    if (!wp_verify_nonce($nonce, 'zigurat_permanently_delete_invoice_' . $trash_id)) {
        $list_error = new WP_Error('invalid_nonce', 'درخواست حذف دائمی معتبر نیست؛ صفحه را تازه‌سازی کنید.');
    } else {
        $permanently_deleted = zigurat_invoice_delete_from_trash($trash_id);
        if (is_wp_error($permanently_deleted)) {
            $list_error = $permanently_deleted;
        } else {
            wp_safe_redirect(add_query_arg('invoice-status', 'permanently-deleted', zigurat_invoice_page_url(array('brand'=>$permanently_deleted->brand, 'view'=>'trash'))));
            exit;
        }
    }
    $view = 'trash';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['save_invoice_settings'])) {
    ...
    if (!wp_verify_nonce($nonce, 'zigurat_save_invoice_settings')) {
        ...
        if (is_wp_error($settings_saved)) {
            ...
            wp_safe_redirect(zigurat_invoice_page_url(array('view'=>'settings','settings-status'=>'saved')) . '#invoice-settings-save-result');
            exit;
        }
    }
}
...
if ($view === 'print' || $view === 'trash_print') {
    nocache_headers();
    $print_invoice_id = isset($_GET['invoice_id']) ? absint($_GET['invoice_id']) : (isset($_GET['id']) ? absint($_GET['id']) : 0);
    $print_is_trash = $view === 'trash_print';
    if ($print_is_trash && !current_user_can('manage_options')) {
        wp_die('اجازه مشاهده سطل زباله را ندارید.', 'خطا', array('response'=>403));
    }
    $invoice = $print_is_trash ? zigurat_invoice_trash_get_invoice($print_invoice_id) : zigurat_invoice_get($print_invoice_id);
    if (!$invoice) {
        wp_die('فاکتور پیدا نشد.', 'خطا', array('response'=>404));
    }
    $print_back_url = $print_is_trash
        ? zigurat_invoice_page_url(array('brand'=>$invoice->brand, 'view'=>'trash'))
        : zigurat_invoice_page_url(array('brand'=>$invoice->brand, 'view'=>'list'));
    ...
    ?><!doctype html><html <?php language_attributes(); ?>><head><meta charset="<?php bloginfo('charset'); ?>"><meta name="viewport" content="width=device-width,initial-scale=1"><title><?php echo esc_html($print_title); ?></title><?php wp_head(); ?><script>window.ziguratInvoicePrintTitle=<?php echo wp_json_encode($print_title, JSON_UNESCAPED_UNICODE); ?>;document.title=window.ziguratInvoicePrintTitle;function ziguratInvoiceMillimeter(){var probe=document.createElement('span');probe.style.cssText='position:absolute;visibility:hidden;width:100mm;height:1px;';document.body.appendChild(probe);var value=probe.getBoundingClientRect().width/100;probe.remove();return value||3.7795275591;}function ziguratInvoiceHeightAtWidth(sheet,invoice,widthMm){sheet.style.setProperty('width',widthMm+'mm','important');void sheet.offsetHeight;var sheetBox=sheet.getBoundingClientRect();var invoiceBox=invoice.getBoundingClientRect();return Math.max(invoice.scrollHeight,invoiceBox.height)+(invoiceBox.top-sheetBox.top);}function ziguratPrepareInvoicePrint(){document.title=window.ziguratInvoicePrintTitle;var sheet=document.querySelector('.invoice-print-sheet');var invoice=document.querySelector('.invoice-document');if(!sheet||!invoice)return;invoice.classList.remove('is-multipage-print','is-multipage-landscape','is-multipage-portrait');var originalWidth=sheet.style.getPropertyValue('width');var originalPriority=sheet.style.getPropertyPriority('width');var millimeter=ziguratInvoiceMillimeter();var landscapeHeight=ziguratInvoiceHeightAtWidth(sheet,invoice,281);var portraitHeight=ziguratInvoiceHeightAtWidth(sheet,invoice,194);if(originalWidth)sheet.style.setProperty('width',originalWidth,originalPriority);else sheet.style.removeProperty('width');void sheet.offsetHeight;var landscapeMultipage=landscapeHeight>(210*millimeter)+2;var portraitMultipage=portraitHeight>(297*millimeter)+2;invoice.classList.toggle('is-multipage-landscape',landscapeMultipage);invoice.classList.toggle('is-multipage-portrait',portraitMultipage);var isPortrait=window.matchMedia&&window.matchMedia('print and (orientation: portrait)').matches;invoice.classList.toggle('is-multipage-print',isPortrait?portraitMultipage:landscapeMultipage);}window.addEventListener('beforeprint',ziguratPrepareInvoicePrint);window.addEventListener('afterprint',function(){var invoice=document.querySelector('.invoice-document');if(invoice)invoice.classList.remove('is-multipage-print','is-multipage-landscape','is-multipage-portrait');});function ziguratPrintInvoice(){document.title=window.ziguratInvoicePrintTitle;window.requestAnimationFrame(function(){window.setTimeout(function(){ziguratPrepareInvoicePrint();window.print();},100);});}</script></head><body class="invoice-print-body" data-print-filename="<?php echo esc_attr($print_title); ?>"><div class="invoice-print-toolbar no-print"><a href="<?php echo esc_url($print_back_url); ?>">بازگشت به فهرست</a><?php if (!$print_is_trash && !zigurat_invoice_is_locked($invoice)): ?><a href="<?php echo esc_url(zigurat_invoice_page_url(array('brand'=>$invoice->brand,'view'=>'form','type'=>$invoice->document_type,'edit'=>$invoice->id))); ?>">اصلاح فاکتور</a><?php endif; ?><?php if (!$print_is_trash): ?><a class="invoice-copy-action" href="<?php echo esc_url(zigurat_invoice_page_url(array('brand'=>$invoice->brand,'view'=>'form','type'=>$invoice->document_type,'copy_from'=>$invoice->id))); ?>">کپی</a><?php endif; ?><button type="button" onclick="ziguratPrintInvoice()">چاپ / ذخیره PDF</button></div><main class="invoice-print-sheet"><?php get_template_part('template-parts/invoice-document', null, array('invoice'=>$invoice)); ?></main><?php wp_footer(); ?></body></html><?php
    exit;
}
...
if ($editing_invoice) {
    ...
    $candidate_copy = zigurat_invoice_get($copy_from_id);
    if ($candidate_copy) {
        $copy_source = $candidate_copy;
        $brand = $copy_source->brand;
        $document_type = $copy_source->document_type;
        $view = 'form';
    } elseif (!$form_error) {
        $form_error = new WP_Error('invalid_copy_source', 'سند مبدأ برای کپی معتبر نیست.');
    }
}
$copy_origin = $editing_invoice && !empty($editing_invoice->copied_from_invoice_id)
    ? zigurat_invoice_get($editing_invoice->copied_from_invoice_id)
    : null;
...
<main class="invoice-admin-page <?php echo $brand === 'unofficial' ? 'invoice-admin-page--unofficial' : ''; ?>"><div class="container">
    ...
    <?php if ($view === 'settings' && current_user_can('manage_options')): ?>
        <?php
        $seller_labels = array('name'=>'نام فروشنده','national_id'=>'شماره ملی/ثبت','economic_no'=>'شماره اقتصادی','province'=>'استان','city'=>'شهر','postal_code'=>'کدپستی','phone'=>'تلفن');
        ...
        <section class="invoice-workspace invoice-settings-page">
            ...
            <form class="invoice-editor invoice-settings-form" method="post" action="#invoice-settings-save-result" enctype="multipart/form-data">
                ...
                <fieldset><legend>شماره بعدی اسناد</legend><p class="invoice-settings-help">عددی را وارد کنید که سند بعدی با آن ثبت شود. شماره باید در فهرست اسناد فعال خالی باشد؛ شماره فاکتورهای موجود قابل تکرار نیست.</p><div class="invoice-fields invoice-fields--4">
                ...
                <?php if ($settings_error): ?><div class="invoice-notice invoice-save-result is-error" id="invoice-settings-save-result" role="alert"><?php echo esc_html($settings_error->get_error_message()); ?></div><?php elseif (!empty($_GET['settings-status']) && $_GET['settings-status']==='saved'): ?><div class="invoice-notice invoice-save-result is-success" id="invoice-settings-save-result" role="status">تنظیمات اولیه فاکتورها ذخیره شد.</div><?php else: ?><span id="invoice-settings-save-result" aria-hidden="true"></span><?php endif; ?>
            </form>
        </section>
    <?php elseif (!$brand): ?>
        <section class="invoice-brand-picker" aria-labelledby="invoice-section-title"><div class="invoice-admin-heading"><span>صدور آنلاین اسناد فروش</span><h1 id="invoice-section-title">بخش فاکتور</h1><p>نوع فاکتور را انتخاب کنید. شماره هر سند هنگام اولین ذخیره به‌صورت خودکار و غیرتکراری ثبت می‌شود.</p></div><div class="invoice-brand-cards"><a class="invoice-brand-card invoice-brand-card--official" href="<?php echo esc_url(zigurat_invoice_page_url(array('brand'=>'official','view'=>'list'))); ?>"><strong>فاکتور رسمی زیگورات</strong><small>با مشخصات ثبتی، مالیات ارزش افزوده و اطلاعات پرداخت</small></a><a class="invoice-brand-card invoice-brand-card--unofficial" href="<?php echo esc_url(zigurat_invoice_page_url(array('brand'=>'unofficial','view'=>'list'))); ?>"><strong>فاکتور غیررسمی دیاموند</strong><small>مناسب پیش‌فاکتور و فاکتورهای فروشگاهی</small></a></div></section>
    <?php else: ?>
        <section class="invoice-workspace">
            ...
            <nav class="invoice-tabs no-print <?php echo current_user_can('manage_options') ? 'invoice-tabs--admin' : ''; ?>"><a class="<?php echo $view==='form'&&$document_type==='proforma'?'is-active':''; ?>" href="<?php echo esc_url(zigurat_invoice_page_url(array('brand'=>$brand,'view'=>'form','type'=>'proforma'))); ?>">ثبت پیش‌فاکتور</a><a class="<?php echo $view==='form'&&$document_type==='invoice'?'is-active':''; ?>" href="<?php echo esc_url(zigurat_invoice_page_url(array('brand'=>$brand,'view'=>'form','type'=>'invoice'))); ?>">ثبت فاکتور</a><a class="<?php echo $view==='list'?'is-active':''; ?>" href="<?php echo esc_url(zigurat_invoice_page_url(array('brand'=>$brand,'view'=>'list'))); ?>">نمایش لیست فاکتورها</a><?php if (current_user_can('manage_options')): ?><a class="<?php echo $view==='trash'?'is-active':''; ?>" href="<?php echo esc_url(zigurat_invoice_page_url(array('brand'=>$brand,'view'=>'trash'))); ?>">سطل زباله</a><?php endif; ?></nav>

            <?php if ($view === 'form'): ?>
                <?php
                $form_source = $editing_invoice ?: ($source_proforma ?: ($correction_source ?: $copy_source));
                ...
                $field = static function ($key, $default='') use ($editing_invoice, $source_proforma, $branch_source, $correction_source, $copy_source) {
                    ...
                    if ($copy_source && !in_array($key, array('issue_date','status','paid_amount'), true) && isset($copy_source->$key)) return $copy_source->$key;
                    return $default;
                };
                $default_tax_rate = ($brand === 'official' && $document_type === 'proforma')
                    ? 0
                    : zigurat_invoice_default_tax_rate($brand);
                if ($_SERVER['REQUEST_METHOD']==='POST') {
                ...
                <form class="invoice-editor <?php echo current_user_can('manage_options') ? '' : 'invoice-editor--payment-readonly'; ?>" method="post" action="#invoice-save-result" data-invoice-editor data-invoice-unsaved="<?php echo $form_error ? '1' : '0'; ?>">
                    ...
                    <div class="invoice-editor__title"><div><span><?php echo $editing_invoice?'اصلاح سند':($correction_source?'اصلاحیه جدید':($branch_source?'انشعاب جدید':($source_proforma?'تبدیل پیش‌فاکتور به فاکتور':($copy_source?'کپی سند':'سند جدید')))); ?></span><h2><?php echo esc_html(zigurat_invoice_document_label($document_type)); ?></h2></div><strong data-invoice-number data-base-number="<?php echo esc_attr($editing_invoice && empty($editing_invoice->parent_invoice_id) ? zigurat_invoice_format_number($editing_invoice->document_number) : ''); ?>">شماره: <span data-invoice-number-value class="<?php echo ($editing_invoice || $branch_source) ? 'invoice-number' : ''; ?>" <?php echo ($editing_invoice || $branch_source) ? 'dir="ltr"' : ''; ?>><?php echo esc_html($header_number); ?></span></strong></div>
                    ...
                    <div class="invoice-editor-bottom">
                        ...
                        <fieldset><legend>محاسبات</legend><div class="invoice-fields"><label>تخفیف کلی<input name="discount" type="text" inputmode="numeric" value="<?php echo esc_attr($field('discount',0)); ?>" data-money></label><label>حمل و بسته‌بندی<input name="shipping" type="text" inputmode="numeric" value="<?php echo esc_attr($field('shipping',0)); ?>" data-money></label><label>ضریب بالاسری/سود پیمانکار (%)<input name="overhead_rate" type="number" min="0" max="100" step="0.01" value="<?php echo esc_attr($field('overhead_rate',0)); ?>" data-clear-on-focus></label><?php if ($brand === 'official'): ?><label>ضریب بیمه (%)<input name="insurance_rate" type="number" min="0" max="100" step="0.01" value="<?php echo esc_attr($field('insurance_rate',0)); ?>" data-clear-on-focus></label><?php endif; ?><label>مالیات ارزش افزوده (%)<input name="tax_rate" type="number" min="0" max="100" step="0.01" value="<?php echo esc_attr($field('tax_rate',$default_tax_rate)); ?>" data-clear-on-focus></label><?php if ($document_type === 'invoice'): ?><label>مبلغ پرداخت‌شده<input name="paid_amount" type="text" inputmode="numeric" value="<?php echo esc_attr($field('paid_amount',0)); ?>" data-money></label><?php else: ?><input type="hidden" name="paid_amount" value="0"><?php endif; ?></div><dl class="invoice-live-totals"><div><dt>جمع اقلام</dt><dd data-subtotal>۰ ریال</dd></div><div><dt>بالاسری/سود پیمانکار</dt><dd data-overhead>۰ ریال</dd></div><?php if ($brand === 'official'): ?><div><dt>بیمه</dt><dd data-insurance>۰ ریال</dd></div><?php endif; ?><div><dt>مالیات</dt><dd data-tax>۰ ریال</dd></div><div><dt>جمع کل</dt><dd data-grand-total>۰ ریال</dd></div><div><dt>مانده</dt><dd data-balance>۰ ریال</dd></div></dl></fieldset>
                    </div>
                    <div class="invoice-editor-actions no-print"><button type="submit">ذخیره <?php echo esc_html(zigurat_invoice_document_label($document_type)); ?></button><?php if($editing_invoice): ?><a href="<?php echo esc_url(zigurat_invoice_page_url(array('view'=>'print','id'=>$editing_invoice->id))); ?>" target="_blank" rel="noopener">مشاهده و چاپ</a><a class="invoice-copy-action" href="<?php echo esc_url(zigurat_invoice_page_url(array('brand'=>$editing_invoice->brand,'view'=>'form','type'=>$editing_invoice->document_type,'copy_from'=>$editing_invoice->id))); ?>">کپی</a><?php if(zigurat_invoice_can_branch($editing_invoice)): ?><a class="invoice-branch-action" href="<?php echo esc_url(zigurat_invoice_page_url(array('brand'=>$editing_invoice->brand,'view'=>'form','type'=>$editing_invoice->document_type,'branch_from'=>$editing_invoice->id))); ?>">ایجاد فرعی بعدی</a><?php endif; ?><?php endif; ?></div>
                    <?php if ($form_error): ?><div class="invoice-notice invoice-save-result is-error no-print" id="invoice-save-result" role="alert"><?php echo esc_html($form_error->get_error_message()); ?></div><?php elseif (!empty($_GET['invoice-status']) && $_GET['invoice-status'] === 'saved'): ?><div class="invoice-notice invoice-save-result is-success no-print" id="invoice-save-result" role="status"><?php echo esc_html(zigurat_invoice_document_label($document_type)); ?> ذخیره شد.</div><?php else: ?><span id="invoice-save-result" aria-hidden="true"></span><?php endif; ?>
                </form>
            <?php elseif ($view === 'trash' && current_user_can('manage_options')): ?>
                <?php get_template_part('template-parts/invoice-trash', null, array('brand'=>$brand, 'list_error'=>$list_error)); ?>
            <?php else: ?>
                <?php
                ...
                <?php if ($list_error): ?><div class="invoice-notice is-error" role="alert"><?php echo esc_html($list_error->get_error_message()); ?></div><?php elseif (!empty($_GET['invoice-status']) && $_GET['invoice-status'] === 'saved-locked'): ?><div class="invoice-notice is-success">فاکتور ذخیره و قفل شد.</div><?php elseif (!empty($_GET['invoice-status']) && $_GET['invoice-status'] === 'deleted'): ?><div class="invoice-notice is-success">فاکتور حذف شد، به سطل زباله رفت و شماره آن آزاد شد.</div><?php endif; ?>
                        ...
                        <?php if ($invoice_list['items']): foreach ($invoice_list['items'] as $row): ?>
                            ...
                            <tr id="invoice-row-<?php echo (int) $row->id; ?>" class="invoice-list-row--<?php echo esc_attr($row->document_type); ?> <?php echo !empty($row->number_suffix) ? 'invoice-list-row--branch' : ''; ?> <?php echo $row->document_type === 'invoice' && ($row->payment_status ?? '') === 'settled' ? 'invoice-list-row--settled' : ''; ?>" data-invoice-selectable data-grand-total="<?php echo esc_attr((int) $row->grand_total); ?>" data-tax-amount="<?php echo esc_attr((int) $row->tax_amount); ?>" data-paid-amount="<?php echo esc_attr((int) $row->paid_amount); ?>" data-balance="<?php echo esc_attr((int) $row->balance); ?>" tabindex="0" aria-selected="false">
                                ...
                                <td>
                                    ...
                                    <a class="invoice-copy-action" href="<?php echo esc_url(zigurat_invoice_page_url(array('brand'=>$row->brand,'view'=>'form','type'=>$row->document_type,'copy_from'=>$row->id))); ?>">کپی</a>
